...
const truncate = value => +value.toString().slice(-1)

const reduceFirst = (acc, value, i) =>
...
export const cycle = (input, iterations = 1) => {
  ...
  for (let i = 0; i < iterations; i++) {
    for (let d = 0; d < curr.length; d++) {
      const chunks = d
        ? [curr.slice(0, d), ...$.chunk(curr.slice(d), d + 1)]
        : curr
      const reduce = d ? reduceAny : reduceFirst
      const value = truncate(chunks.reduce(reduce, 0))
