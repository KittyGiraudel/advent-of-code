...
const detect = (input, y, max) => {
  ...
  if (y && !max) {
    ...
    const maxX = $.last(sorted).position[0] + $.last(sorted).radius
  ...
  const candidate = sensors.reduce((candidate, sensor, _, sensors) => {
    ...
    const top = [position[0], position[1] - radius - 1]
    const right = [position[0] + radius + 1, position[1]]
    const bottom = [position[0], position[1] + radius + 1]
    const left = [position[0] - radius - 1, position[1]]
