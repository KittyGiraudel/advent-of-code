...
const isWithinSensorRange = ({ position, radius }: Sensor, coords: Coords) =>
  $.manhattan(position, coords) <= radius

const isOutOfRange = (signals: Sensor[], coords: Coords) =>
  signals.every(signal => !isWithinSensorRange(signal, coords))

const isValidCandidate = (signals: Sensor[], coords: Coords, max: number) =>
...
export const detect = (input: string[], y: number, max?: number) => {
  const data = input.map(line => $.safeMatch(line, /-?\d+/g).map(Number))
  ...
  if (y && !max) {
    ...
    const candidate = sensors.reduce<Coords | null>(
      (candidate, sensor, _, sensors) => {
