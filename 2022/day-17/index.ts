...
type Blocks = Array<Coords>
type Rock = Array<string>

let DEBUG = false
const WIDTH = 7
const ROCKS: Array<Array<string>> = [
  '####'.split('\n'),
  '.#.\n###\n.#.'.split('\n'),
  '..#\n..#\n###'.split('\n'),
  '#\n#\n#\n#'.split('\n'),
...
const isRowEmpty = (row: Array<string>) => row && row.join('') === EMPTY_ROW
...
export const tetris = (input: string, count: number = 2022) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    while (true) {
      const sideways = moveSideways(
        grid,
        blocks,
        input[sideIndex.next().value!]
      )
