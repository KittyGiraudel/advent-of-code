...
const recordGuardId = (event, index, log) => {
  if (!event.id) {
...
const processEvent = (event, index, log) => {
  if (event.type === 'falls asleep') {
    log[index + 1].start = event.date.getUTCMinutes()
  }
...
const aggregateEvents = (acc, event) => {
  if (!(event.id in acc)) {
    acc[event.id] = []
  }
...
const formatData = ([guardId, minutes]) => ({
  id: +guardId,
  duration: minutes.length,
  counters: Object.entries($.count(minutes))
    .map(([minute, occurrences]) => ({ minute, occurrences }))
