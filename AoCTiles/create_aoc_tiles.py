...
def handle_day(day: int, year: int, solutions: list[str], html: HTML, day_scores: DayScores | None):
    ...
    with html.tag("a", href=str(solution_link)):
        alt = f"Day {day} {year}: "
        if (day_scores is not None and day_scores.rank1 != None):
            alt += "Complete" if day_scores.rank2 != None else "Partially complete"
        else:
            alt += "Incomplete"
        html.tag("img", closing=False, src=str(day_graphic_path), alt=alt, width=TILE_WIDTH_PX)
