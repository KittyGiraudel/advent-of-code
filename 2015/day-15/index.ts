...
const getPropertyScore =
  (ratios: Permutation, ingredients: Array<Array<number>>) =>
  (_: unknown, pi: number) => {
...
export const run = (input: Array<string>, calories?: number) => {
  ...
  const permutations = PERMUTATIONS[
    input.length as keyof typeof PERMUTATIONS
  ] as Array<Permutation>
