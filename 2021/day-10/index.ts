...
type Line = { type: string; character?: string; opened?: Array<string> }
...
const getLinesFromType = (lines: Array<string>, type: string): Line[] =>
  lines.map(processLine).filter(line => line.type === type)

export const getCorruptionScore = (lines: Array<string>): number =>
  getLinesFromType(lines, 'CORRUPTED').reduce(
    (score, { character }) => score + CORRUPTION_SCORE_MAP[character],
    0
  )

export const getCompletionScore = (lines: Array<string>): number => {
